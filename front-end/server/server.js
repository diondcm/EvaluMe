...
// Backend service URL for analyze-image and status endpoints
const backendServiceUrl = process.env.BACKEND_SERVICE_URL || 'https://write-analyzer-23tlv4ogmq-uc.a.run.app';
...
async function getIdToken(targetAudience) {
    try {
        ...
        console.warn('Failed to get ID token for authentication:', error.message);
        return null;
    }
}

// Proxy for /analyze-image/ and /status/ endpoints
// IMPORTANT: This must come BEFORE body parsers to handle raw streams
app.use(['/analyze-image', '/status'], async (req, res, next) => {
    try {
        // req.url includes the path after the matched route (e.g., '/' or '/:id')
        // req.originalUrl includes the full path (e.g., '/analyze-image/' or '/status/123')
        const targetUrl = `${backendServiceUrl}${req.originalUrl}`;
        console.log(`Backend Proxy: Forwarding ${req.method} request from ${req.originalUrl} to ${targetUrl}`);
        console.log(`Backend Proxy: Content-Type: ${req.headers['content-type']}`);
        ...
        // Add authentication token if running on Cloud Run (service-to-service auth)
        const idToken = await getIdToken(backendServiceUrl);
        if (idToken) {
            forwardHeaders['Authorization'] = `Bearer ${idToken}`;
            console.log('Backend Proxy: Added authentication token');
        } else {
            console.log('Backend Proxy: No authentication token available (running locally?)');
        }
        ...
        // For POST/PUT/PATCH, forward the raw request stream to preserve multipart/form-data
        ...
        console.log(`Backend Proxy: Received ${apiResponse.status} ${apiResponse.statusText} from backend`);
...
// IMPORTANT: Body parsers must come AFTER the proxy routes above
